...
parser.parser_info['dict_name']='EngCom.koi'
...
parser.set_dict_info("basename", "EngCom")
parser.begin()
parser.set_dict_info('full_name', 'Современный свободный словарь компьютерных терминов.')
...
for line in parser.f.readlines():
		...
		try:
			...
			print "ERROR (There is too many spaces in the line really, run check_dict.sh : " + res
			sys.exit(1)
		data=data.rstrip()
		new_data=''	
		if read_abbr==1:
